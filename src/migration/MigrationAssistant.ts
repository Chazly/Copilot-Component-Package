...
export class MigrationAssistant {
  ...
  migrateToAI(legacyConfig: CopilotConfig): AICopilotConfig {
    try {
      ...
      const migratedConfig: AICopilotConfig = {
        ...
        // Apply sensible defaults for new features
      ...
      throw new MigrationError(
        ...
        'Failed to migrate legacy configuration to AI configuration'
      )
    }
  }
  ...
  upgradeToPhase5(config: AICopilotConfig): AICopilotConfig {
    const upgraded: AICopilotConfig = { ...config }
    ...
    upgraded.metadata = {
      ...upgraded.metadata,
      upgradedToPhase5: new Date().toISOString(),
      previousPhase: detectConfigPhase(config)
    }
  ...
  executeMigration(config: CopilotConfigType, plan: MigrationPlan): AICopilotConfig {
    ...
    try {
      for (const step of plan.steps) {
        switch (step.action) {
          ...
          default:
            throw new MigrationError(
              step.action,
              `Unknown migration action: ${step.action}`,
              `Failed to execute step ${step.step}: ${step.title}`
            )
        }
      }
      ...
      if (!isAICopilotConfig(currentConfig)) {
        throw new MigrationError(
          'final_validation',
          'Migration did not result in valid AICopilotConfig',
  ...
  private getDefaultAIConfig(): Partial<AICopilotConfig> {
    return {
      tools: [],
      contextSources: [],
      persona: {
  ...
  private getDefaultEnterpriseMemoryConfig() {
    return {
      scopes: {
        enabled: ['session', 'user', 'organization'] as ("session" | "user" | "organization" | "global" | "ephemeral")[],
    ...
    return {
      ...
      integrations: {
        ...
        api: {
          ...
          authentication: {
            required: true,
            methods: ['apikey' as const, 'bearer' as const]
          }
        }
      },
      compliance: {
  ...
  private getPhaseUpgradeStep(phase: number, stepNumber: number): MigrationStep {
    const phaseSteps: Record<number, Omit<MigrationStep, 'step'>> = {
      2: {
  ...
  private getMigrationBenefits(fromPhase: number, toPhase: number): string[] {
...
export const migrateToAI = (config: CopilotConfig): AICopilotConfig => 
  migrationAssistant.migrateToAI(config)

export const migrateToLatest = (config: CopilotConfigType): AICopilotConfig => 
  migrationAssistant.migrateToLatest(config)

export const getMigrationPlan = (config: CopilotConfigType, targetPhase?: number): MigrationPlan => 
  migrationAssistant.getMigrationPlan(config, targetPhase)

export const previewMigration = (config: CopilotConfigType) => 
  migrationAssistant.previewMigration(config)

export const validateMigration = (config: CopilotConfigType) => 
  migrationAssistant.validateMigration(config) 